# 03 — SID Fit Coach Knowledge → Behavior Architecture

> [!IMPORTANT]
> **REFERENCE ONLY — KHÔNG CÒN LÀ RUNTIME AUTHORITY (từ 2026-09-05).**
>
> ### Thay đổi: Gộp routing layer vào `13-knowledge-routing-and-behavior.md`
>
> * **Vấn đề cũ (file 03 + file 11 song song):** Routing layer bị tách thành hai file deployed cùng lúc. File 03 chứa behavioral taxonomy (14 `BK-*` classes), node registry (24 `KN-*` clusters), knowledge-to-behavior matrix, router (`KR-01`–`KR-08`) và checkpoint (`KBC-01`–`KBC-08`). File 11 chứa crosswalk K01–K16 map intent → category filename. Runtime phải retrieve **hai** files để hoàn thành **một** công việc "câu hỏi này → đọc file nào". Điều này tăng retrieval noise (hai file semantic-gần-nhau cạnh tranh similarity score), tăng context bloat và tạo risk conflict giữa hai file về meaning.
> * **Cập nhật (2026-09-05):** Hợp nhất toàn bộ nội dung 03 + 11 thành **một routing authority duy nhất** là `13-knowledge-routing-and-behavior.md`. File 13 giữ nguyên 100% evidence status, safety dependency, decision gate, KBC logic, source conflict policy, coverage matrix và missing-knowledge table của hai file gốc — chỉ loại bỏ overlap về cấu trúc và đặt crosswalk K01–K16 lên §1 làm entry point routing.
> * **Trạng thái file này:** GIỮ NGUYÊN, không xóa, dùng làm design reference và audit trail cho P3. **Không upload vào Knowledge nữa.** Nếu file 13 khác file này về scientific meaning, file 13 thắng và phải ghi routing defect để review.

**Phase:** P3 — Knowledge → Behavior  
**Authority:** `design-conversation-architect.md` V3  
**Dependencies:** `01-output-architecture.md`, `02-reasoning-safety-rules.md`  
**Knowledge status:** 16 categories; `PASS_WITH_CAVEATS`  
**Runtime status:** SUPERSEDED BY `13-knowledge-routing-and-behavior.md`

> Knowledge được phép ảnh hưởng phản hồi chỉ sau context, safety, epistemic và decision-precondition gates. Retrieval không đồng nghĩa answer.

## 1. Design Goal và Input Contract

```text
User signal → decision/intent → safety pre-filter
→ behavioral knowledge class → candidate node
→ context + evidence check → reasoning effect
→ Output Contract → checkpoint → handoff
```

**Input:** current signal, confirmed/fresh context, active decision, safety class, requested depth.  
**Transformation:** chọn tập node tối thiểu đủ thay đổi question/decision/explanation/action.  
**Output:** traceable behavior; không dump source.  
**Gate:** `KBC-01`–`KBC-08`.  
**Handoff:** semantic conditions cho P4; chưa khóa state ID.

## 2. Knowledge Registry

Node ID ở P3 đại diện cho **behavioral cluster**; các concept/function gốc vẫn giữ nguyên trong decomposition và được liệt kê ở cột Source nodes.
...
## 3. Behavioral Knowledge Taxonomy
...
## 10. Evaluation, P4 Handoff và Verdict

### 10.1 Upstream dependency coverage

- Output contracts: `OA-01`, `OA-02`, `OA-03`, `OA-04`, `OA-05`, `OA-06`, `OA-07`, `OA-08`, `OA-09`, `OA-10`, `OA-11`, `OA-12`, `OA-13`, `OA-14`, `OA-15`.
- Reasoning modes: `RM-01`, `RM-02`, `RM-03`, `RM-04`, `RM-05`, `RM-06`, `RM-07`.
- Decision gates: `DP-01`, `DP-02`, `DP-03`, `DP-04`, `DP-05`, `DP-06`, `DP-07`, `DP-08`, `DP-09`, `DP-10`, `DP-11`, `DP-12`, `DP-13`.
- Safety classes: `SC-A`, `SC-B`, `SC-C`, `SC-D`.
...
- [x] Đủ 16 source categories trong registry.
- [x] Knowledge phân loại theo behavioral role, không chỉ tên sách.
- [x] Mapped node có trigger, context, behavioral effect, output và safety dependency.
- [x] Retrieval không tự trở thành answer/recommendation.
- [x] Có source conflict policy và `NEEDS SOURCE REVIEW` fallback.
- [x] Có coverage, orphan và missing-knowledge analysis.
- [x] Risk-sensitive nodes reference P2 guards.
- [x] Có `KBC-01`–`KBC-08` PASS/REVISE gate.
- [x] Không tạo state IDs hoặc Master Instruction sớm.

### 10.3 Handoff schema cho P4

```yaml
knowledge_dependency:
  behavioral_class: BK-xx
  candidate_nodes: [KN-xx]
  semantic_entry_conditions: []
  required_context: []
  freshness_requirements: []
  reasoning_modes: [RM-xx]
  decision_preconditions: [DP-xx]
  safety_dependencies: [SC-x, SAFE-xx, TRG-xx]
  allowed_outputs: [OA-xx]
  forbidden_when: []
  fallback: clarify | generalize | caveat | redirect
  checkpoint_status: PASS | REVISE
```

P4 dùng semantic entry/exit conditions này để xây state; không đổi node evidence hoặc P2 safety outcome.

## Verdict

# READY_FOR_P4

1. 24 behavioral node clusters bao phủ đủ 16 categories mà không tạo mega-summary.
2. Node được nối với context, reasoning, decision, safety, output và fallback.
3. Source conflict/mixed evidence không bị hợp nhất thành certainty giả.
4. Coverage và missing knowledge được ghi minh bạch; không có orphan behavior blocking.
5. P4 có thể dùng stable dependencies để thiết kế state/transition.
